...
# todo: load latest combined metadata file
...
cancer_type_psi_table <- psi_combined |>
  dplyr::filter(event_type == params$event_type) |>
  tidyr::pivot_longer(
    cols = contains("_PSI"),
    names_pattern = "(.*)_PSI",
    names_to = "Run",
...
Doesn't make sense to include these until we've resolved the redundant IDs
...
- Just do for skipped exon events (let people filter for favorite splice event)

- Need to convert gene IDs to HUGO IDs

- For the biology vignette's next step, I would export pos_id, gene_id, dPSI, padj, columns of `for_plot_p_dPSI_table` for ORA
