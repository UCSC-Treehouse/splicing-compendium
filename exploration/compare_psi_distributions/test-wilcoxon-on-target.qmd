...
for_plot_p_dPSI_table <- for_plot_p_dPSI_table |>
  dplyr::mutate(gene_label = ifelse(
    # genes are only labeled if it is significantly differentially spliced and in the top 15 most differentially spliced in magnitude
    gene_id %in% top_15_genes$gene_id & 
      colors %in% c("sig_and_diff_increase", "sig_and_diff_decrease"),
    gene_id,
    # if not in these criteria, leave label blank
    ""))
