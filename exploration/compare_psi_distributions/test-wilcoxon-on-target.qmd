...
```
### Write diffsplice results to output
```{r write_output}
readr::write_tsv(wilcox_test_events)
```
...
The above output should include all columns expected to be exported from splice compendium differential splice analysis.
...
for_plot_p_dPSI_table <- p_dPSI_table |>
  ...
  dplyr::mutate(
    colors = dplyr::case_when(
      dPSI > params$dPSI & minus_log_padj > -log10(params$padj) ~ "sig_and_diff_increase",
      dPSI < -params$dPSI & minus_log_padj > -log10(params$padj) ~ "sig_and_diff_decrease"
    )
  )
...
```{r all_events_volcano}
#| label: fig-all_events_volcano_plot
#| fig-width: 11
#| fig-height: 8

ggplot(for_plot_p_dPSI_table, aes(x = dPSI, y = minus_log_padj)) +
  geom_point(aes(colour = as.factor(colors))) +
        scale_color_manual(
          values = c(
            "sig_and_diff_increase" = "orange",
            "sig_and_diff_decrease" = "blue"
            )
          ) +
  # label top 15 significant genes with splice events of highest dPSI magnitide
  ggrepel::geom_text_repel(aes(label = gene_id), max.overlaps = Inf, box.padding = 0.5) +
  
  # add boundary lines to mark dPSI and padj thresholds
  geom_vline(xintercept = params$dPSI, linetype = "dashed", colour = "black") +
  geom_vline(xintercept = -params$dPSI, linetype = "dashed", colour = "black") +
  geom_hline(yintercept = -log10(params$padj), linetype = "dashed", colour = "black") +
  xlab("dPSI") +
