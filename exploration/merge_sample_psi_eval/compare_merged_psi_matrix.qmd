...
# usage: join_tables(merged_events_df, combined_events_df)
...
# usage: summarize_shared_events(joined_table)
...
Read in event tables, filter only for columns to compare (pos_id, psi, gene_id), and merge them into one table of all events
...
Compare tables
```{r}
merged_vs_combined_table <- join_tables(merged_events_df, combined_events_df)
```
