...
```{r remove_unused_dfs}
rm(merged_shared_id_counts_diff_df)
rm(combined_shared_id_counts_diff_df)
rm(deduplicated_combined_junctions)
rm(merged_junctions)
```
...
SRR4419554 has the highest number of junctions with mismatched counts for chromosome 1
