...
Roll this into a function so it can be used for the merged_only junctopmns

```{r}
#| label: fig-samples_with_nonzero_counts_combined_only_ids

combined_only_ids_samples_summary <- deduplicated_combined_junctions |>
...
hist(combined_only_ids_samples_summary$n_counts_over_zero,
     main = paste("Distribution of samples with counts > 0 per junction ID"),
     xlab = "Number of samples with counts > 0 for junction IDs only in combined table")
```
