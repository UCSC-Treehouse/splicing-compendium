...
```
Excluding retained intron event types, PSI values are identical in events where pos_id, event_type, gene_id, and label are the same.

#### Do RI position IDs that are shared between tables (including same gene ID) have the same PSI values?

```{r rep1_vs_rep2_ri_pos_ids_identical}
# extract position ids of events that are shared between tables
...
In replicate compendium runs, retained intron events with the same `gene_id` and `pos_id` fields have identical PSI values.
This finding supports our thinking that the difference in RI event PSI calculation stems from how exon-intron junctions are processed in the compendum workflow.