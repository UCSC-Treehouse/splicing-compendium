...
genes_rep1_vs_rep2_events_summary <- rep1_vs_rep2_example_table |> dplyr::summarise(
    .by = c(event_type, label),
    # count number of events in each event type and annotation category
    total = dplyr::n(),
    shared_count = sum(shared_event),
...
genes_rep1_vs_rep2_events_summary |> 
  # no need to look at event types that are all the same
  dplyr::filter(
    shared_percent != 100
  ) |>
...
Print example of events with same pos_ids, put into different genes

```{r}
# construct table of rep1 and rep2 events and label by whether event is shared
rep1_vs_rep2_example_table <- dplyr::full_join(
  rep1_psi_table,
  rep2_psi_table,
  by = c("event_type", "pos_id", "gene_id", "label"),
  # label PSI values by table they came from
...
based on the previous version of this notebook where I printed out examples of cases where positions IDs are the same but being assigned different genes, I wonder how much of differing PSI values stem from cases where junctions in the same position IDs are assigned different genes when Shiba calculates PSI.
