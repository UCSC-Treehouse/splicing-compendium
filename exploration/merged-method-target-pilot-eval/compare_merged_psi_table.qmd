...
target_pilot_workflow_results_dir <- file.path(exploration_dir, "withlogs_v1_pilot_rep1", "psi")
rep2_target_pilot_results_dir <- file.path(exploration_dir, "withlogs_v1_pilot_rep2", "psi")
...
# rep1 event files
rep1_events_path <- file.path(target_pilot_workflow_results_dir, "v1_events_rep1")
rep2_events_path <- file.path(rep2_target_pilot_results_dir, "v1_events_rep2")
...
# event files
events_rep1 <- read_in_events(rep1_events_path)
events_rep2 <- read_in_events(rep2_events_path)
...
Because all differences in the PSI tables stem from pos_ids being assigned different genes, 
I suspect the difference comes from the GTF generation or merge steps in the workflow.
...
Since differences in the PSI tables from the `rep1 vs. combined` and `rep1 vs. rep2` comparisons stem from positions being assigned to different gene IDs in annotated events, 
first check what GTF entries for features in an example unshared pos_id look like.
...
e.g. I would have expected there to be nonzero GTF features assigned to different genes.
