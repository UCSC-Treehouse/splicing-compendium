...
This is event ID chr9:35660647-35660648, which has "chr9;chr9" for the chromosome
```{r}
SRR1559031_problems |> dplyr::slice(434391, 434392, 434393)
```

chr9:35660647-35660648, our problem junction, also has high read counts ( > 10 ) so it seems like an important junction to be able to keep.
...
Importantly, the ID is the same: chr9:35660647-35660648
...
```
With how we currently merge junctions, the problem junction appears as two separate events that are missing values in the other samples

## Proposed new junction merging method:

Use ID to extract chr, start, and end fields in events with weird concatenated fields
...
merged_junctions <- purrr::map(junction_paths, \(file) {
        ...
        \(x) {
          sapply(x, \(val) {
            ...
            if (!stringr::str_detect(val, ";")) {
              # split string by semicolon and take the first field
              # this assumes the junctions separated by ; are always duplicates
              deduplicated <- stringr::str_split_i(val, ";", 1)
            }
          })
        }
      )
  ...
  # merge junctions tables from multiple samples
  # the resulting table separates junction counts
  # from each sample by columns with the sample ID
  purrr::reduce(
    \(x, y) dplyr::full_join(
      x, y,
