...
```
No, when the junction is in incorrect bed format, there is no corresponding duplicate 'fixed' version.
...
merged_junctions <- purrr::map(junction_paths, \(file) {
  ...
  # check for duplicate junction IDs and fail if any are found
  dup_ids <- junctions$ID[duplicated(junctions$ID)]
...
merged_junctions |>
  dplyr::filter(ID == "chr9:35660647-35660648")
```
In this case the junctions stay unmerged because the merging code stopped with a warning from the invalid entry