...
# Read in file paths
merged_psi_table_dir <- file.path("merged_results")
...
merged_junctions <- read_junctions(junction_paths)
...
combined_only <- setdiff(shiba_psi_table$pos_id, merged_psi_table$pos_id)
...
shiba_psi_table <- shiba_psi_table |>
  dplyr::mutate(
    combined_only = pos_id %in% unique_shiba,
    merged_only = pos_id %in% unique_merged
  )

# print summary of how many unique events with values that are novel vs. unannotated
unique_shiba_psi_table |> 
  dplyr::summarise(.by = c(label), 
                   count = dplyr::n())
...
Print the number of annotated vs. unannotated events in each event type only in the together table
...
Print the number of annotated vs. unannotated events in each event type only in the separate table
