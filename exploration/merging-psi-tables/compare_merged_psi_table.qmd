...
There are 1,586 more junctions in the separate junctions table than the combined table.
One possible explanation is that these extra junctions represent positions that are very close together (in other words, a fewer number of junctions would have been called if there were better read support from all possible junction regions like in the combined bedfile).
...
There is a very small fraction of junctions that are only found in the combined or separate junctions tables. 
...
# create tables containing only junction IDs found in the separate or combined tables
...
For the junctions that exceed the PSI threshold, check if they are close to any junctions in the combined table
```{r}
separate_only_junctions_table |>
  dplyr::filter(below_ten == "no")
```
Check combined junctions table
```{r}
...
together_only_list <- setdiff(shiba_psi_table$pos_id, merged_psi_table$pos_id)

# produce dataframe of elements in merged splice results dataframe but not in the reference df
separate_only_list <- setdiff(merged_psi_table$pos_id, shiba_psi_table$pos_id)
```
...
Together / combined table

```{r}
# filter for pos_ids unique to the shiba dataframe
shiba_psi_summary <- shiba_psi_table |>
  dplyr::mutate(
    together_only = pos_id %in% together_only_list,
...
# print summary of how many unique events with values that are novel vs. unannotated
  dplyr::summarise(.by = c(label, together_only, separate_only), 
                   count = dplyr::n()) |>
