...
Filter out junctions that are too short to be incorporated into PSI calculation
...
compare_junctions <- dplyr::full_join(
  ...
  # filter for junctions > 6bp for columns with an underscore
  dplyr::filter(junc_len > 6) |>
  # convert NAs to 0 in the separate junctions columns
  dplyr::mutate(
...
# Check if the separate and combined values are the same for each junction after filtering
identical(compare_junctions$SRR601500_combined, compare_junctions$SRR601500_separate)
identical(compare_junctions$SRR604528_combined, compare_junctions$SRR604528_separate)
```
Check where NAs might have been introduced to understand warning
```{r}
nas <- compare_junctions[!complete.cases(compare_junctions),]
dim(nas)
```
