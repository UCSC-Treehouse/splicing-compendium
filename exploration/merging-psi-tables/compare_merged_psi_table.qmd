...
Since the minimum length of a junction to be used is 6, filter for junctions that are greater than 6 bp for comparison.
...
filtered_separate_junctions <- separate_junctions |>
  ...
  tidyr::replace_na(list(SRR604528 = 0, SRR604528 = 0)) |>
...
```
Once we filter for junctions long enough to be included in splice calculation, all IDs are shared in both junction tables.
...
In conclusion, for this test case of 2 samples, once we filter out junctions smaller than 6bp in length and replace NAs with 0, the counts for both combined and separate tables become identical
