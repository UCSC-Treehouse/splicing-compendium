...
To better understand patterns behind what PSI values are missing in each
method, this notebook categorizes PSI values into “match categories”,
based on what the value (or NA type) for that position is in each
method’s PSI table.
...
important_categories <- c(
  ...
  "NA in separate, quantified in combined",
  "NA in combined, quantified in separate",
  "PSI quantified in combined, dropped in separate table")
...
quantified in combined, dropped in separate table” categories. These
categories of match types have explanations for why PSI values may be
missing.
...
- Events we expect are real but are missed in the separate method due to
  how events are defined in the GTF: PSI quantified in combined, dropped
  in separate table

In this table, most mismatches (35%) stem from cases where the splice
event is dropped in the separate table due to only one gene being
detected in the GTF, but there are too few junction counts for a splice
event in that gene for the event to be assigned a PSI value in the
combined table. This category is not concerning, as these events would
not appear in the “canonical” Shiba method.

The most concerning mismatch types in this table are “PSI quantified in
combined, dropped in separate table” (9%) and “PSI only quantified in
combined table” (4%), as these represent missing splice events due to
differences between each method. Although the overall percent appears
small, only 16% of PSI values are quantified in both tables, so we are
missing about as many events as we can detect with this method.
...
When we apply the min_samples filter, there is a smaller ratio of
concerning mismatches in PSI values between the methods. 47% of PSIs are
shared, compared to 3% of “PSI quantified in combined, dropped in
separate table”. There are no splice events in this filtered dataframe
in the “PSI only quantified in combined table” category.
