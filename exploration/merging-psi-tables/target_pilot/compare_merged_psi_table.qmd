...
- NA: NA values from merging the separate tables (samples have a NA value after this step if there are genes dropped by Shiba for those samples because there is only one transcript detected for that gene)

NAs from the combined table
- -1: NA values from Shiba for the separate tables (may be due low coverage)

NAs from merging combined and separate tables
- -2: NAs coming from splice events that are only found in one analysis method (combined vs. separate)
...
combined_splice_results_paths <- shiba_psi_paths |>
  purrr::map(\(path){
    readr::read_tsv(path, col_types=readr::cols(.default = "c")) |>
      ...
      # convert PSI values to numeric and replace NA values with -1
