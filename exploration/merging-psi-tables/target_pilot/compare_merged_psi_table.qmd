...
* How many unannotated event types remain if we only filter for two cancer types before trying to drop all NAs in the splice table?
...
Inflated values in the separate tables may be due to The capturing of unannotated event types suffers more from merging separate PSI tables.
