...
* How many NA values of different NA types are there in each type of splice table? How are they distributed across events? I am more concerned with the merge NAs (-2), as there is no good way to replace them (they could either be 0 or 1). Another concern is events that are only found in the combined table, or only found in the separate table, as there is no easy way to correct for those.
...
-   Of splice events with high numbers of NA values, how many of these events only have a numeric PSI value in one (or a very low amount of) samples? Can the number of NA values be reduced in the separate tables method if we filter for events with a minimum number of samples?
-   How well correlated are the PSI values and NAs in each splice table type? Assuming the Shiba NAs (-1) are from low read support, do the separate and combined tables always call the same events as a Shiba NA? Merge NAs (-2) should arise from events dropped by Shiba due to lack of alternative transcripts for a gene, so we expect these NAs to be 0 or 1.
...
but my takeaway is that we need to unify the tables somehow (whether by converting all Shiba NAs in the separate tables to 0, or some other method),
Or we need to not drop 

Filter for minmum of 5 samples with numeric PSI values
```{r}
#| label: fig-complete_splice_events_breakdown
#| fig-width: 11
#| fig-height: 8
...
plot_event_summary(all_numeric_events_summary )
```
This actually looks all right to me
...
na_comparison_samples <- long_all_events |>
  dplyr::select(pos_id, sample, method, PSI) |>
  tidyr::pivot_wider(
    names_from = method,
    values_from = PSI
  ) 
...
corresponding to splice events dropped in the separate tables due to lack of transcripts for a gene but was captured in the combined splice table.
...
Plot how many of each of these match categories are present in the pilot tables
```{r}
...
```{r}
#| label: fig-exploratory_two_cancer_types_numeric_splice_events_breakdown
#| fig-width: 11
#| fig-height: 8
...
cancers_list <- filtered_target_metadata |>
  dplyr::filter(study_name %in% c("TARGET: Kidney\\, Clear Cell Sarcoma of the Kidney (CCSK)", 
