...
Although the majority of splice events (inclusive of NA values) are shared between methods, there are about twice as many splice events only in the separate method's table than the combined method.
...
`n_quantified_separate` and `n_quantified_combined` columns have min values of 5, so filtering appears to be correct.
...
- Although most events are shared after applying this filter and we still have unannotated events, we also still have events only found in the separate table.
...
```
There appear to be more events dropped across more samples in the combined results compared to the separate results.
...
I do not plot this for the combined table because they are all 0 (we only define what is dropped due to insufficient transcript representation based off what transcripts are missing when individual separate tables are merged).
...
```
This category of missing values are inflated in unannotated events in the separate table.
...
We care most about "PSI quantified in both", "NA in separate, quantified in combined", "NA in combined, quantified in separate", and "PSI quantified in combined, dropped in separate table" categories.
...
- Events we expect are real but are missed in the separate method due to how events are defined in the GTF: PSI quantified in combined, dropped in separate table

In this table, most mismatches (35%) stem from cases where the splice event is dropped in the separate table due to only one gene being detected in the GTF,
...
The most concerning mismatch types in this table are "PSI quantified in combined, dropped in separate table" (9%) and "PSI only quantified in combined table" (4%),
...
47% of PSIs are shared, compared to 3% of "PSI quantified in combined, dropped in separate table".
...
The PSI values of the "gene dropped" NAs span the full range of PSI values, so we miss out entirely on a wide range of values using the separate method.
