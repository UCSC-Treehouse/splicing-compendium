...
sample_level_psi_comparison <- function(
    long_df,
    min_sample_value
    ) {
  long_df |>
  # Select for relevant columns in long sample-level table for matchup analysis
    ...
    # select for samples with numeric PSI values in a minimum number of samples
    dplyr::filter(
      
    ) |>
    # we are not interested in comparing events only found in one table (there will be no match to compare it to)
    tidyr::drop_na() |>
