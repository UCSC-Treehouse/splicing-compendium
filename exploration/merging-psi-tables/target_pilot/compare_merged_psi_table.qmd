...
The same trend regarding unannotated events is seen here as in @fig-num_quantified_psi.
...
These events should correspond to splice events that only have one transcript annotated in the sample's GTF.
...
Similar to @fig-num_low_read_na, I mainly see that these missing values are present more in the unannotated events, where the event is only quantified in a small number of samples.
