...
# long df output filtered by min samples
min_sample_long_df_output <- file.path(target_pilot_output, "min_filtered_long_psi_df.rds")
```
...
if (use_cached_long) {
  long_all_events <- readr::read_rds(long_all_events)
} else {
...
Check if result exists in cache
```{r}
use_cached_filtered_long <- file.exists(min_sample_long_df_output) && params$use_cache
```

```{r}
if (use_cached_filtered_long) {
  long_min_sample_all_events <- readr::read_rds(min_sample_long_df_output)
} else {
...
# filtered long df
saveRDS(long_min_sample_all_events, file = min_sample_long_df_output)
```
