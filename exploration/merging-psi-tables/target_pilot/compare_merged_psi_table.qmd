...
test <- all_events_summary |>
  dplyr::select(event_type, label, combined_only_count, separate_only_count, total, combined_only_percent, separate_only_percent) |>
...
ggplot(long_summary_df, aes(fill = event_type, x = category, y = count)) +
    ...
    facet_wrap(vars(label, event_type), scales = "free_y") +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
...
ggplot(long_summary_df, aes(fill = event_type, x = category, y = percent)) +
    ...
    facet_wrap(vars(label, event_type)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme(strip.text = element_text(size = 15))
```

I think of these as "percent of each event category missed" 
In the worst cases, 40% of unannotated events may be only in the separate only table (ALE events) and 30% of unannotated events might be only in the combined table (FIVE events)
...
It's surprising that there are a handful of unannotated retained intron events shared across all samples, but my takeaway is that we need to unify the tables somehow (whether by converting all shiba NAs in the separate tables to 0, or some other method) because almost all unannotated events are only found in one sample or tumor type.
...
```{r}
# this requires more messing with the code (reading in sample sheet and adding cancer type to splice table) so I will run the idea by folks tomorrow and implement later. Or implement if I get through the rest of today

# we probably want the sample metadata 
```
