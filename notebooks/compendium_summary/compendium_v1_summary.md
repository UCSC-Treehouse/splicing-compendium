...
gtex <- ggplot(for_summary_gtex_df, aes(y = plot_tissue_type, fill = sex)) +
  geom_bar(
    position = "stack"
  ) +
  labs(
  ...
  # allow tissue types to be plotted in same order as age distribution plots
  scale_y_discrete(
    limits = rev(
      c(
        "EBV lymphocytes",
...
target <- ggplot(cleaned_target_df, aes(y = plot_tissue_type, fill = sex)) +
  geom_bar(
    position = "stack"
  ) +
  labs(
