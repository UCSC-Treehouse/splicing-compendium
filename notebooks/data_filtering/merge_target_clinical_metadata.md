...
Clinical metadata is obtained from the [UCSC Xena browser metadata
hub](https://xenabrowser.net/datapages/?hub=https://gdc.xenahubs.net:443).
Metadata related to clinical attributes of the sample are split into two
files for each TARGET cancer type in the linked page.
...
Prior to combining survival and phenotype metadata to existing TARGET
accession metadata, the survival and phenotype information is filtered
...
The sample IDs are harmonized in these metadata files by removing the
last substring of “biospecimen_repository_sample_id” by “-”.

Then, the TARGET accessions metadata is combined with the accessions
sheet and the TARGET SRA metadata is filtered for accessions
corresponding to samples that were downloaded and processed.

## Combine TARGET clinical and survival metadata

Combine each TARGET cancer type’s clinical and survival metadata and
filter for accessions that are present in the compendium
...
corresponds to one entry in the clinical metadata. In these cases, the
sample comes from the same patient. These cases are handled by
duplicating the clinical metadata for the samples that come from the
same patient (don’t assume one-to-one relationship in merge).
...
All samples in the compendium have clinical data accounted for. Some ALL
expansion phase 2 samples appear to be in the Xena browsers [ALL Phase
3](https://xenabrowser.net/datapages/?host=https%3A%2F%2Fgdc.xenahubs.net&dataset=TARGET-ALL-P3.clinical.tsv&allSamples=true&removeHub=https%3A%2F%2Fxena.treehouse.gi.ucsc.edu%3A443)
metadata, instead of the phase 2 metadata as expected.
