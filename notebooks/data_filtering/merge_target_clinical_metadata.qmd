...
Check row 2421 - this corresponds to one AML patient with two RNA-seq samples in the TARGET dbGaP accessions metadata, 
...
In this case, it makes sense to me to preserve both matches (two samples from the same patient) since the clinical information should mostly be the same for samples from the same patient 
...
Columns excluded due to having multiple values per row/ID: primary_site.project, entity.submitter_id.annotations, notes.annotations, submitter_id.annotations, classification.annotations, entity_id.annotations, annotation_id.annotations, entity_type.annotations,
...
target_clinical_accessions <- target_sra_metadata |>
  dplyr::left_join(target_metadata, by = dplyr::join_by(
    target_sample_id == sample),
    relationship = "one-to-one"
    ) |>
