...
# Download script for a subset of TARGET bulk RNA-seq for pilot pediatric cancer splicing analysis
...
# Define log files
log_file="${log_dir}/${current_datetime}_GTEX_TARGET_subset_download.txt"
error_log_file="${log_dir}/${current_datetime}_download_errors_GTEX_TARGET_subset.txt"

# Redirect stdout and stderr to log file and print to stdout
exec > >(tee $log_file) 2>&1

# Run filtering script to obtain accession IDs to download
quarto render filter_gtex_target_sra.qmd
