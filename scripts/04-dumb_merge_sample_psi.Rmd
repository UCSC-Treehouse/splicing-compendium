...
Function for merging SE tables together
```{r}

read_se_table <- function(sample_paths, psi_table_name) {
  ...
  purrr::map(sample_paths, \(one_sample_path) {
    ...
    file_paths <- file.path(one_sample_path, psi_table_name)
...
```{r}
merged_se_table <- read_se_table(sample_paths, "PSI_SE.txt")
```
