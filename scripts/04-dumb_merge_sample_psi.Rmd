...
samples <- readr::read_tsv(samples_file, col_types = c(.default = "c")) |>
  dplyr::pull(sample) |>
  as.list() |>
  # subset
  head(10)
...
read_se_table <- function(sample_paths) {
  ...
  file_paths <- file.path(sample_paths, "PSI_SE.txt")
...
read_afe_ale_five_three_table <- function(sample_paths, psi_event_file) {
  ...
  # construct paths to each psi output for each sample
  file_paths <- file.path(sample_paths, psi_event_file)
...
```{r}
new_se_table <- read_se_table(sample_paths) 
new_afe_table <- read_afe_ale_five_three_table(sample_paths, "PSI_AFE.txt")
new_ale_table <- read_afe_ale_five_three_table(sample_paths, "PSI_ALE.txt")
new_five_table <- read_afe_ale_five_three_table(sample_paths, "PSI_FIVE.txt")
new_three_table <- read_afe_ale_five_three_table(sample_paths, "PSI_THREE.txt")

```

identical?
```{r}
all.equal(new_ale_table, merged_ale_table)
all.equal(new_five_table, merged_five_table)
```
